...
def build_dedupe_plan(
    ...
    *,
    ...
    with progress.step(
        f"Scanning {len(roots)} root(s) for media files",
    ...
    with progress.step(
        "Grouping files by name and size",
        done=lambda: f"Found {len(candidate_groups)} possible duplicate group(s)",
    ):
        for media in files:
            by_name_size[media.name_size_key].append(media)
        candidate_groups = [
            (name_size_key, group) for name_size_key, group in sorted(by_name_size.items()) if len(group) >= 2
        ]
    ...
    candidate_file_count = sum(len(group) for _, group in candidate_groups)
    if candidate_file_count:
        with progress.step(
            f"Hashing {candidate_file_count} possible duplicate file(s)",
            done=lambda: f"Hashed {hashed_candidates} possible duplicate file(s)",
        ):
            for name_size_key, group in candidate_groups:
        ...
        progress.log("No same-name same-size duplicate candidates found")

    with progress.step(
        "Building duplicate move plan",
        done=lambda: f"Planned {duplicate_files} duplicate file move(s)",
    ):
